...
def get_settings(force_field: str):
    """
    ...
    ----------
    force_field: str
        The string of the force field contents or the installed openff force field to use in the protocol.
    """
    ...
    settings.forcefield_settings.small_molecule_forcefield = force_field
    # Only run one repeat per input json file
    settings.protocol_repeats = 1
    return settings


def get_settings_charge_changes(force_field: str):
    """
...
def get_force_field(force_field: pathlib.Path) -> str:
    """
    Load the local force field and get the string representation of it.

    Parameters
    ----------
    force_field: pathlib.Path
        The path to the local force field which should be loaded.
    ...
    return ForceField(force_field.as_posix()).to_string()
...
@click.option(
    '--force-field',
    type=click.Path(dir_okay=False, file_okay=True, path_type=pathlib.Path),
    help="The path to a SMIRNOFF style force field which should be used in the protocol.",
    default=None
)
def run_inputs(ligands, pdb, cofactors, output, force_field):
    """
    ...
    force_field: Optional[pathlib.Path]
        The optional path to the local SMIRNOFF style force field which should be used in the protocol.
    ...
    # if we have a local force field extract the string version
    if force_field is not None:
        force_field_str = get_force_field(force_field=force_field)
    else:
        # hard code the default force field
        force_field_str = "openff-2.2.0"
    ...
    for mapping in ligand_network.edges:
        ...
        if abs(charge_difference) > 1e-3:
            ...
            rfe_settings = get_settings_charge_changes(force_field=force_field_str)
        else:
            rfe_settings = get_settings(force_field=force_field_str)
