...
def gen_ligand_network(smcs, central_ligand_num):
    """
    Creates the radial ligand network using the KartografAtomMapper with
    the ligand defined by `central_ligand_num` as the central ligand.

    Parameters
    ----------
    smcs : list[SmallMoleculeComponents]
      List of SmallMoleculeComponents of the ligands.
    central_ligand_num : int
      The index of the ligand in smcs to be used as the center of the radial
      graph.

    Returns
    -------
    openfe.LigandNetwork
      The generated radial LigandNetwork.
    """
    print("INFO: Generating Radial Network")
    mapping_filters = [
...
@click.option(
    ...
    type=int,
    required=True,
    help="Index (0th based) of the ligand in the SDF file to be used as the center of the radial graph",
)
@click.option(
    '--output',
...
def run_inputs(ligands, pdb, cofactors, central_ligand_num, output):
    """
    ...
    central_ligand_num : int
      The index (0th based) of the ligand in the SDF file
      to be used as the center of the radial graph.
    output: pathlib.Path
    ...
    ligand_network = gen_ligand_network(smcs, central_ligand_num)
