        ...
        for k in res["protocol_result"]["data"]:
            ...
            if name in edge_count:
                if (
                    ...
                    or not "solvent" in stateA.components
                ):
...
def do_taping(
    ...
    input_alchemical_network: str = None,
):
    ...
    duck_taped_network = ducktape_networks(
        ligand_sub_networks=ligand_sub_networks, n_connecting_edges=3
    )
    print(
...
def cli_do_taping(
    ...
    do_taping(
        result_files_regex=results_regex_path,
